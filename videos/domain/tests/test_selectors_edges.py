...
def test_list_for_user_with_public_ids(monkeypatch, owner_user):
    ...
    serialized = selectors_public.list_for_user_with_public_ids(
...
def test_get_video_stream_returns_manifest_from_database(
    monkeypatch, owner_user, tmp_path
):
    video = _create_video(owner_user, is_published=True)
    ...
    result = selectors.get_video_stream(
        movie_id=video.id, resolution="720p", user=owner_user
    )
